...
BulkSMS::BulkSMS(String name, String token)
{
  this.user_name = name;
...
void BulkSMS::_send_req(String text, String operation)
{
  ...
  // do post request
  http.begin(BULKSMS_WEBSERVICE_URL);
  http.addHeader("Content-Type", "application/x-www-form-urlencoded");

  // HTTP_RET_CODE = http.POST(url);
  HTTP_RET_CODE = http.POST(data);
  data = "";

  response = http.getString();      // get response payload
  http.end();

  // handle bulksms errors
  handleErrors()
...
String BulkSMS::status()
{
  // return status message, err | OK
  return this.stat;
}